...
def smd_inject_anomalies(
    ...
    if injection_method == "None":
        ...
        raise ValueError(
            f"injection_method = {injection_method} not supported!")
...
def smd_pipeline(
    ...
    **kwargs,
    ...
    #####     Load Data     #####
    # print(data_dir)
    train_set, test_set = ncad.datasets.smd(path=data_dir)
    ...
    train_set = TimeSeriesDataset(
        ncad.utils.take_n_cycle(scaler(train_set), len(train_set)))
    test_set = TimeSeriesDataset(
        ncad.utils.take_n_cycle(scaler(test_set), len(test_set)))
    ...
    if distance == "cosine":
        ...
        distance = ncad.model.distances.BinaryOnX1(
            rep_dim=embedding_rep_dim, layers=1)
    ...
    checkpoint_cb = ModelCheckpoint(
        ...
        dirpath=model_dir,
        filename="ncad-model-" + exp_name +
        "-{epoch:02d}-{train_loss_step:.4f}",
        save_top_k=1,
        mode="min",
    )
    ...
    evaluation_result = trainer.test(ckpt_path='best', datamodule=data_module)
    evaluation_result = evaluation_result[0]

    # Save evaluation results
    if evaluation_result_path is not None:
        path = evaluation_result_path
        path = PosixPath(path).expanduser() if str(
            path).startswith("~") else Path(path)
    ...
    print(f"ncad on smd dataset finished successfully!")
